@echo off
REM Batch script to clean bin, obj, and TestResults folders for entire project
setlocal enabledelayedexpansion

REM Change to script directory
cd /d "%~dp0"

powershell -Command "Write-Host '=== Cleaning Entire Project ===' -ForegroundColor Cyan"
echo.
...
if !COUNT! gtr 0 (
    ...
    powershell -Command "Write-Host 'Removed bin folders' -ForegroundColor Green"
) else (
...
if !OBJ_FOUND! gtr 0 (
    ...
    powershell -Command "Write-Host 'Removed obj folders' -ForegroundColor Green"
) else (
    if !OBJ_COUNT! equ 0 (
        echo No obj folders found
    ) else (
        echo.
        powershell -Command "Write-Host 'All obj folders removed' -ForegroundColor Green"
    )
)
...
for /d /r . %%d in (TestResults) do (
    if exist "%%d" (
        ...
        rd /s /q "%%d" 2>nul
        if exist "%%d" (
            powershell -Command "Write-Host 'Failed to remove: %%d' -ForegroundColor Red"
        ) else (
            powershell -Command "Write-Host 'Successfully removed: %%d' -ForegroundColor Green"
        )
    )
)
if !COUNT! gtr 0 (
    ...
    for /d /r . %%d in (TestResults) do (
        if exist "%%d" (
            set /a REMAINING+=1
            powershell -Command "Write-Host '  Folder remains: %%d' -ForegroundColor Yellow"
        )
    )
    if !REMAINING! gtr 0 (
        echo.
        powershell -Command "Write-Host '!REMAINING! TestResults folder(s) remain' -ForegroundColor Yellow"
        powershell -Command "Write-Host '  Files may be locked by another process' -ForegroundColor Yellow"
    ) else (
        echo.
        powershell -Command "Write-Host 'Removed TestResults folders' -ForegroundColor Green"
    )
) else (
...
if !COUNT! gtr 0 (
    echo Found !COUNT! .vs folder(s)
    echo Removed .vs folders
) else (
...
REM Run dotnet clean on all solution files
echo.
powershell -Command "Write-Host '=== Running dotnet clean on all solutions ===' -ForegroundColor DarkCyan"
for /r "OJT_Laboratory_Project" %%f in (*.sln) do (
    if exist "%%f" (
        echo Cleaning: %%f
        dotnet clean "%%f" --verbosity quiet
    )
)
powershell -Command "Write-Host 'dotnet clean completed' -ForegroundColor Green"
...
if !COUNT! gtr 0 (
    ...
    powershell -Command "Write-Host 'Removed remaining obj folders' -ForegroundColor Green"
) else (
...
if !COUNT! gtr 0 (
    ...
    if !REMAINING! gtr 0 (
        ...
        echo.
        powershell -Command "Write-Host 'Removed remaining TestResults folders' -ForegroundColor Green"
    )
) else (
...
echo.
powershell -Command "Write-Host '=== Clean completed successfully! ===' -ForegroundColor Green"
exit /b 0
