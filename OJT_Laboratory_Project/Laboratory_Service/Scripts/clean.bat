@echo off
REM Batch script to clean bin, obj, and Visual Studio files for Laboratory_Service
setlocal enabledelayedexpansion

REM Change to project root directory
cd /d "%~dp0\.."

powershell -Command "Write-Host '=== Cleaning Laboratory_Service ===' -ForegroundColor DarkCyan"
echo.
...
if !OBJ_FOUND! gtr 0 (
    if !OBJ_COUNT! equ 0 (
        echo.
        powershell -Command "Write-Host 'Found !OBJ_FOUND! obj folder(s)' -ForegroundColor DarkCyan"
    )
...
if exist ".vs" (
    rd /s /q ".vs" 2>nul
    powershell -Command "Write-Host 'Removed .vs folder' -ForegroundColor DarkGreen"
) else (
...
if !COUNT! gtr 0 (
    powershell -Command "Write-Host 'Removed !COUNT! .user file(s)' -ForegroundColor DarkGreen"
) else (
...
if !COUNT! gtr 0 (
    powershell -Command "Write-Host 'Removed !COUNT! .suo file(s)' -ForegroundColor DarkGreen"
) else (
...
dotnet clean Laboratory_Service.sln --verbosity quiet
