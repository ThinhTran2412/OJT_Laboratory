@echo off
REM ====================================================================
REM Script to UPDATE database migrations for Laboratory_Service
...
if %ERRORLEVEL% equ 0 (
    powershell -Command "Write-Host ''"
    powershell -Command "Write-Host '✓ Laboratory_Service database updated successfully!' -ForegroundColor DarkGreen"
    exit /b 0
) else (
    powershell -Command "Write-Host ''"
    powershell -Command "Write-Host '✗ Laboratory_Service database update failed!' -ForegroundColor DarkRed"
    exit /b 1
)
