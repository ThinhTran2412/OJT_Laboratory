@echo off
REM ====================================================================
REM Script to UPDATE database migrations for Monitoring_Service
...
powershell -Command "Write-Host '  Updating Monitoring_Service Database (DEVELOPMENT)' -ForegroundColor Cyan"
...
if %ERRORLEVEL% equ 0 (
    powershell -Command "Write-Host ''"
    powershell -Command "Write-Host '✓ Monitoring_Service database updated successfully!' -ForegroundColor DarkGreen"
    exit /b 0
) else (
    powershell -Command "Write-Host ''"
    powershell -Command "Write-Host '✗ Monitoring_Service database update failed!' -ForegroundColor DarkRed"
    exit /b 1
)
