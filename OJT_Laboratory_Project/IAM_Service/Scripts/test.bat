@echo off
REM Batch script to run tests for IAM_Service
...
dotnet clean IAM_Service.sln >nul 2>&1
...
dotnet build IAM_Service.sln --configuration Debug --no-restore >nul 2>&1

REM Prepare folders
if not exist "IAM_Service.Application.UnitTests\TestResults\coverage" mkdir "IAM_Service.Application.UnitTests\TestResults\coverage"
if not exist "IAM_Service.Application.UnitTests\TestResults\CoverageReport" mkdir "IAM_Service.Application.UnitTests\TestResults\CoverageReport"

powershell -Command "Write-Host 'Running tests with coverage...' -ForegroundColor DarkCyan"
dotnet test IAM_Service.Application.UnitTests/IAM_Service.Application.UnitTests.csproj ^
  --collect:"XPlat Code Coverage" ^
  --results-directory ./IAM_Service.Application.UnitTests/TestResults ^
...
set COVERAGE_FILE=IAM_Service.Application.UnitTests\TestResults\coverage\coverage.cobertura.xml
if not exist "%COVERAGE_FILE%" (
    for /r "IAM_Service.Application.UnitTests\TestResults" %%f in (coverage.cobertura.xml) do (
        if exist "%%f" (
            set COVERAGE_FILE=%%f
            goto :coverage_found
        )
...
set HTML_REPORT=IAM_Service.Application.UnitTests\TestResults\CoverageReport\index.html
