@echo off
REM ====================================================================
REM Git Commit and Push Script for IAM_Service
REM Features:
...
REM Check if git repository exists in IAM_Service folder only
if not exist ".git" (
    echo Error: Not a git repository in IAM_Service!
    echo Current directory: %CD%
    echo IAM_Service must have its own .git folder.
    exit /b 1
)

echo.
echo ==============================================================
echo   Git Commit and Push - IAM_Service
echo ==============================================================
echo.
...
REM Get git root (should be IAM_Service folder)
set GIT_ROOT=%CD%
...
REM ====================================================================
REM Step 2: Check Git Status (only IAM_Service files)
REM ====================================================================
cd /d "%GIT_ROOT%"
...
echo [Step 2/6] Checking git status for IAM_Service...
echo   Remote: !REMOTE_NAME! - !REMOTE_URL!
...
if !FILE_COUNT! equ 0 (
    echo   [WARNING] No changes in IAM_Service to commit
    del status.log 2>nul
    del error.log 2>nul
...
echo.
echo   [OK] Found !FILE_COUNT! file(s) with changes in IAM_Service
del status.log 2>nul
del error.log 2>nul
echo.

REM ====================================================================
REM Step 3: Add Changes (only IAM_Service)
REM ====================================================================
echo [Step 3/6] Adding IAM_Service changes...
echo   Remote: !REMOTE_NAME! - !REMOTE_URL!
echo   Branch: !TARGET_BRANCH!
if not "!TRACKING_REMOTE!"=="" (
    echo   Tracking: !TRACKING_REMOTE!/!TRACKING_BRANCH!
...
echo   [OK] IAM_Service changes added to staging
echo   Staged files:
...
git commit -m "[IAM_Service] !COMMIT_MESSAGE!" 2>error.log
if %ERRORLEVEL% neq 0 (
...
REM ====================================================================
echo ==============================================================
echo   Summary - IAM_Service
echo ==============================================================
echo   Service: IAM_Service
echo   Remote: !REMOTE_NAME! - !REMOTE_URL!
echo   Branch: !TARGET_BRANCH!
if not "!TRACKING_REMOTE!"=="" (
    echo   Tracking: !TRACKING_REMOTE!/!TRACKING_BRANCH!
)
echo   Commit: [IAM_Service] !COMMIT_MESSAGE!
echo ==============================================================
